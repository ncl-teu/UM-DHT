...
/*
class AbstractRTTComparator{
public:
    virtual ~AbstractRTTComparator(){};
    virtual int compare(const KademliaBucketEntry &lhs, const KademliaBucketEntry &rhs) const = 0;
};

public RTTComparator : public AbstractRTTComparator
{
public:
int compare(const KademliaBucketEntry& lhs, const KademliaBucketEntry& rhs) const
{
    // return 0 if accuracy is too low


    if (lhs->getRtt() <= rhs->getRtt()) return -1;
    if (lhs->getRtt() > rhs->getRtt()) return 1;
    return 0;
}
};
 ...
 * Kanemitsu
 */
 //class KademliaRTTComparator : public Comparator<>
/*
 class KademliaRTTComparator : public Comparator
 {
 public:
     KademliaRTTComparator(const simtime_t& rtt1, )
     virtual int compare( const simtime_t& lhs, const simtime_t& rhs ) const
     {
         return lhs.compareTo(rhs);
     }
 };
